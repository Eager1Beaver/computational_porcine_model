...
class PorcineCellFactory_b : public AbstractCardiacCellFactory<3>
{
private:
    GenericVarier* mpGenericVarier;
    std::vector<boost::shared_ptr<RegularStimulus> > mRegularStimuli;
...
public:
    PorcineCellFactory_b(		const std::vector<double>& stimulusinfo,
								std::string meshpath,
    ...
    AbstractCardiacCellInterface* CreateCardiacCellForTissueNode(Node<3>* pNode)
    {
		...
		if (relative_position > (1.0 - mLayer1))
			{
				p_cell = new Cellcarro_2011_epi_pig_scratch_4_go1FromCellMLCvodeOpt(mpSolver, mpZeroStimulus);					
			}
		else if(relative_position > (1.0 - mLayer1 - mLayer2))
			{
				p_cell = new Cellcarro_2011_m_pig_scratch_4_go1FromCellMLCvodeOpt(mpSolver, mpZeroStimulus);
			}
		else
			{
				p_cell = new Cellcarro_2011_endo_pig_scratch_4_go1FromCellMLCvodeOpt(mpSolver, mpZeroStimulus);
			}
